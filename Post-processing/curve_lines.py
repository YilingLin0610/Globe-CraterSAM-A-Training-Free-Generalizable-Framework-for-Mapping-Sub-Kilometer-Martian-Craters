...
def extract_lines_with_conditions(line_gdf, raster_matrix, geotrans, thre):
    ...
    copy_idx=[]
    # 遍历每条线
    for idx, line in tqdm(line_gdf.iterrows()):
        #print(line.Name)
        #print(line)
        #print(idx)
        #print(line["fid_copy"])
    ...
    for i, values in enumerate(line_values):
        ...
        if not contains_nan:
            ...
            try:
                #print("yes")
                # 定义参数范围
                R=np.max(x_data) - np.min(x_data)
                p0_no = [np.max(detrended_values)/2, np.max(x_data) / 4, (R / 12) ** 2,
                         np.max(detrended_values)/2, 3*np.max(x_data) / 4, (R / 12) ** 2]
                lower_bounds = [0, 0, 0.1, 0, np.max(x_data) / 2, 0.1]
                upper_bounds = [np.max(detrended_values), np.max(x_data) / 2, R**2 , np.max(detrended_values),
                                np.max(x_data), R**2]

                # 拟合曲线
                params, params_covariance = curve_fit(fixed_function, x_data, detrended_values,p0=p0_no,
                ...
                if(r_squared<0.1):
                    print(str(copy_idx[i]),"R^2",r_squared)
                ...
                # plt.text(0.25 * np.max(x_data), 0.4 * np.max(detrended_values), f'c1 = {c1:.2f}')
                # plt.text(0.25 * np.max(x_data), 0.3 * np.max(detrended_values), f'c2 = {c2:.2f}')
                #plt.text(0.25 * np.max(x_data), 0.4 * np.max(detrended_values), f'ratio = {ratio:.2f}')
                # plt.title(str(idxs[i]))
                # plt.ylim(min(detrended_values),max(detrended_values)+1.5)
                # plt.show()
                #plt.savefig(r"G:\Mars\Mars_SFS_batch\CTX_images_process_C_1\MurrayLab_GlobalCTXMosaic_V01_E076_N76\figures_1\\"+str(copy_idx[i])+".jpg")
