...
def post_process(mosaic_ID, SFS_path,curve_threshold,roundness_thrshold):
    ...
    if latitude>60 or latitude<-60: #Polor region
        ...
        print(gdf_larger_polygon_pro)
        print(len(gdf_larger_polygon_pro))
        if(len(gdf_larger_polygon_pro)>0):
            filter_gdf_larger_polygon_pro = gdf_larger_polygon_pro[
                gdf_larger_polygon_pro['circularity'] >= roundness_thrshold]
            ...
            filter_gdf_larger_polygon_pro = gpd.GeoDataFrame(columns=["id", "name"], geometry=[], crs="EPSG:4326")

            # 保存为 shapefile
            filter_gdf_larger_polygon_pro.to_file(SAM_shapefile_orinigal[0:-4] + "_large_pro.shp")
            filter_gdf_larger_polygon = gpd.GeoDataFrame(columns=["id", "name"], geometry=[], crs="EPSG:4326")
    ...
    if(len(filtered_gdf)>0):
        ...
        if (abs(latitude) > 60):
            reproject_shp_to_polar(SFS_path + "merged_result_update" + "_" + str(curve_threshold) + "_roundness_" + str(
                roundness_thrshold) + ".shp",
                                   SFS_path + "merged_result_update" + "_" + str(curve_threshold) + "_roundness_" + str(
                                       roundness_thrshold) + "_pro.shp", latitude, lon0=0.0)

    else:
        gdf = gpd.GeoDataFrame(columns=["id", "name"], geometry=[], crs="EPSG:4326")

        # 保存为 shapefile
        gdf.to_file(SFS_path + "merged_result_update" + "_" + str(curve_threshold) + "_roundness_" + str(
            roundness_thrshold) + ".shp")
